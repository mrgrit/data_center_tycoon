...
export const EVENTS = [
  {
    ...
    fire(S) {
      ...
      return {
        ...
        ref: c,
        options: [
          { label: '자체 수리 (기술자 필요)', cost: 3_000_000, needStaff: 'tech', minutes: 90,
            note: '기술자가 있으면 싸고 빠르다. 없으면 이 선택지가 막힌다.' },
          { label: '외주 긴급 출동', cost: 12_000_000, minutes: 240,
    ...
    fire(S) {
      ...
      return {
        ...
        options: [
          ...
          { label: '연료 긴급 반입', cost: 8_000_000, minutes: 60, fuel: 1500,
  ...
  },
  {
    ...
    fire(S) {
      ...
      return {
        ...
        options: [
          { label: '긴급 정비', cost: 18_000_000, minutes: 90, fixChiller: true },
  ...
  },
  {
    ...
    fire(S) {
      ...
      return {
        ...
        options: [
          { label: '방어 장비 임대', cost: 15_000_000, minutes: 120, mitigate: true,
            note: '부하를 원래대로 되돌린다.' },
    ...
    fire(S) {
      ...
      return {
        ...
        options: [
          ...
          { label: '가동 중 응급 조치', cost: 9_000_000, minutes: 60, risky: 0.35,
  ...
  },
  {
    ...
    fire(S) {
      return {
        ...
        options: [
          { label: '정식 대응', cost: 5_000_000, minutes: 120, audit: true,
  ...
  },
  {
    ...
    fire(S) {
      ...
      return {
        ...
        options: guarded
          ? [{ label: '정상 복구', cost: 4_000_000, minutes: 60 }]
          : [
              { label: '백업에서 복구', cost: 25_000_000, minutes: 300, note: '느리지만 확실하다.' },
              { label: '협상한다', cost: 60_000_000, minutes: 60, rep: -12,
...
export function maybeFire(S) {
  ...
  /* 120일 시뮬레이션에서 366건이 터졌다 — 하루 3건이면 배울 새가 없고
     복구가 아니라 두더지잡기가 된다. 규모가 클 때 이틀에 한 번쯤으로 낮춘다. */
  if (Math.random() > 0.00021 * scale) return null;
...
export function resolve(S, ev, opt) {
  ...
  if (opt.audit) {
    ...
    if (margin > 0.35) { S.reputation = Math.min(100, S.reputation + 10); log.push({ kind: 'good', msg: '실사 통과 — 평판 +10' }); }
    else { S.reputation = Math.max(0, S.reputation - 6); log.push({ kind: 'bad', msg: '전력 여유 부족이 지적됐다 — 평판 -6' }); }
  }
  ...
  if (opt.risky && Math.random() < opt.risky) {
    const dmg = 20_000_000 + Math.random() * 40_000_000;
    S.cash -= dmg; S.reputation = Math.max(0, S.reputation - 10);
